$(document).ready(function () {
  ...
  function showManageGroups() {
    $.ajax({
      method: "GET",
      url: BASE_URL + "users/manage_group/0",
  ...
  $("#create_new_groupForm").on("submit", function (e) {
    ...
    if (insertType == "Update") {
      ...
      method = "POST";
      link = BASE_URL + "users/rename_group";
    } else {
      method = "POST";
      link = BASE_URL + "users/manage_group/0";
    }
    if (name != "") {
      $.ajax({
        method: method,
  ...
  $("#group_permissions tbody").on("click", ".editPerms", function (e) {
    let id = $(this).attr("GroupID");
    window.location.replace(BASE_URL + "users/edit_group/" + id);
  });
  ...
  function showGroup(id) {
    $.ajax({
      method: "GET",
      url: BASE_URL + "users/manage_group/" + id.toString(),
      headers: { "X-CSRFToken": csrftoken },
      async: false,
      success: function (data) {
        if (!data.isError) {
  ...
  $("#group_permissions tbody").on("click", ".deletGroup", function (e) {
    ...
    Swal.fire({
      ...
      if (e.value) {
        $.ajax({
          method: "DELETE",
          url: BASE_URL + "users/manage_group/" + GroupID.toString(),
          headers: { "X-CSRFToken": csrftoken },
          async: false,
          success: function (data) {
            if (!data.isError) {
