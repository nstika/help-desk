$(document).ready(function () {
  ...
  function RefreshPage() {
    ...
    window.location.replace(
      BASE_URL +
        `recycle/recycle_ticket?DataNumber=${entries}&SearchQuery=${search}&page=${page}`
    );
  }

  $("#tickets_table").on("click", "#tickets", function () {
    const ID = $(this).attr("ticketId").toString();
    let data = new FormData();
    data.append("Type", "Tickets");
      ...
      if (e.value) {
        $.ajax({
          ...
          success: function (data) {
            if (!data.isError) {
              Swal.fire("Recycling!", data.Message, "success").then(function (
                e
              ) {
                location.reload();
              });
